    ...
    <div class="mx-auto max-w-7xl space-y-6">
        ...
        <form method="GET" action="{{ route('incoming-letters.index') }}" class="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm dark:border-slate-800 dark:bg-slate-900">
            ...
            <div class="mt-4 flex flex-wrap gap-2">
                ...
                <a href="{{ route('incoming-letters.index') }}" class="rounded-lg border border-slate-300 px-4 py-2 text-sm font-semibold text-slate-700 hover:bg-slate-50 dark:border-slate-600 dark:text-slate-200 dark:hover:bg-slate-800">Reset</a>
            </div>
        </form>
                ...
                <table class="min-w-full divide-y divide-slate-200 dark:divide-slate-800">
                    <thead class="bg-slate-50 dark:bg-slate-950/60">
                        <tr>
                            ...
                            <th scope="col" class="px-4 py-3 text-left text-xs font-semibold uppercase tracking-wide text-slate-600 dark:text-slate-400">
                                @php
                                    $nextDirection = ($sort === 'tanggal_penerimaan' && $direction === 'asc') ? 'desc' : 'asc';
                                    $qs = array_merge(request()->except('page'), ['sort' => 'tanggal_penerimaan', 'direction' => $nextDirection]);
                                @endphp
                                <a href="{{ route('incoming-letters.index', $qs) }}" class="inline-flex items-center gap-1 hover:text-indigo-600 dark:hover:text-indigo-400">
                                    Tgl. penerimaan
                                    @if($sort === 'tanggal_penerimaan')
                                        <span class="tabular-nums text-[10px]">{{ $direction === 'asc' ? '↑' : '↓' }}</span>
                                    @endif
                                </a>
                            </th>
                        ...
                        @forelse($letters as $letter)
                            <tr class="hover:bg-slate-50/80 dark:hover:bg-slate-800/40">
                                ...
                                <td class="whitespace-nowrap px-4 py-3 text-right text-sm">
                                    <a href="{{ route('incoming-letters.show', $letter) }}" class="font-medium text-indigo-600 hover:text-indigo-800 dark:text-indigo-400">Detail</a>
                                    <span class="mx-2 text-slate-300 dark:text-slate-600">|</span>
                                    <a href="{{ route('incoming-letters.edit', $letter) }}" class="font-medium text-slate-600 hover:text-slate-900 dark:text-slate-400 dark:hover:text-white">Edit</a>
                                    <span class="mx-2 text-slate-300 dark:text-slate-600">|</span>
                                    <form action="{{ route('incoming-letters.destroy', $letter) }}" method="POST" class="inline" onsubmit="return confirm('Hapus surat ini?');">
                                        @csrf
                                        @method('DELETE')
