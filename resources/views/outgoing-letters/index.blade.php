<x-app-layout title="Surat Keluar">
    <div class="mx-auto max-w-7xl space-y-6">
        <div class="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
            ...
            @php
                $letterOrgKeys = array_keys(config('archive.letter_organizations', []));
                $adminCreateOrg = auth()->user()->isAdmin() && in_array(request('organization'), $letterOrgKeys, true) ? request('organization') : null;
            @endphp
        ...
        <form method="GET" action="{{ route('outgoing-letters.index') }}" class="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm dark:border-slate-800 dark:bg-slate-900">
            <div class="grid gap-4 md:grid-cols-5 md:items-end">
                ...
                <div class="md:col-span-2">
                    ...
                    <input id="q" name="q" type="search" value="{{ request('q') }}" placeholder="Nomor surat, penerima, perihal, klasifikasi, indeks (kode atau label seperti A (internal))..." class="mt-1 block w-full rounded-lg border-gray-300 shadow-sm focus:border-violet-500 focus:ring-violet-500 dark:border-slate-700 dark:bg-slate-950 dark:text-white" />
                </div>
                <div>
                    ...
                    <input id="date_from" name="date_from" type="date" value="{{ request('date_from') }}" class="mt-1 block w-full rounded-lg border-gray-300 shadow-sm focus:border-violet-500 focus:ring-violet-500 dark:border-slate-700 dark:bg-slate-950 dark:text-white" />
                    <p class="mt-1 text-[11px] text-slate-500 dark:text-slate-500">Berdasarkan tanggal pengiriman surat (fallback tanggal surat).</p>
                </div>
                <div>
                    ...
                    <input id="date_to" name="date_to" type="date" value="{{ request('date_to') }}" class="mt-1 block w-full rounded-lg border-gray-300 shadow-sm focus:border-violet-500 focus:ring-violet-500 dark:border-slate-700 dark:bg-slate-950 dark:text-white" />
                </div>
            </div>
                    ...
                    <tbody class="divide-y divide-slate-100 dark:divide-slate-800">
                        @forelse($letters as $letter)
                            <tr class="hover:bg-slate-50/80 dark:hover:bg-slate-800/40">
                                @if(auth()->user()->isAdmin())
                                    <td class="whitespace-nowrap px-4 py-3 text-sm font-medium text-slate-700 dark:text-slate-300">{{ config('archive.letter_organizations')[$letter->organization] ?? strtoupper($letter->organization) }}</td>
                                @endif
                        ...
                        @empty
                            <tr>
                                <td colspan="{{ auth()->user()->isAdmin() ? 10 : 9 }}" class="px-4 py-12 text-center text-sm text-slate-500 dark:text-slate-400">Belum ada data surat keluar.</td>
                            </tr>
                        @endforelse
                    </tbody>
                </table>
