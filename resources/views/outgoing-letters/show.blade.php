<x-app-layout title="Detail Surat Keluar">
    <div class="mx-auto max-w-5xl space-y-6">
        <div class="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
            ...
            <div class="flex flex-wrap gap-2">
                @if($outgoingLetter->file_path)
                    <a href="{{ route('outgoing-letters.download', $outgoingLetter) }}" class="inline-flex items-center gap-2 rounded-xl border border-slate-300 bg-white px-4 py-2 text-sm font-semibold text-slate-800 hover:bg-slate-50 dark:border-slate-600 dark:bg-slate-900 dark:text-slate-100 dark:hover:bg-slate-800">
                ...
                <a href="{{ route('outgoing-letters.index', auth()->user()->isAdmin() ? array_filter(['organization' => $outgoingLetter->klasifikasi === 'bersama' ? 'bersama' : $outgoingLetter->organization]) : []) }}" class="inline-flex items-center rounded-xl border border-slate-300 px-4 py-2 text-sm font-semibold text-slate-700 hover:bg-slate-50 dark:border-slate-600 dark:text-slate-200 dark:hover:bg-slate-800">Kembali</a>
            </div>
        </div>
        ...
        <div class="grid gap-6 lg:grid-cols-2">
            <dl class="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm dark:border-slate-800 dark:bg-slate-900 sm:p-8">
                <div class="grid gap-6">
                    ...
                    <div>
                        ...
                        <dd class="mt-1 text-sm text-slate-800 dark:text-slate-200">{{ $outgoingLetter->penerima }}</dd>
                    </div>
                    <div>
                        <dt class="text-xs font-semibold uppercase tracking-wide text-slate-500 dark:text-slate-400">Perihal</dt>
                        <dd class="mt-1 text-sm leading-relaxed text-slate-700 dark:text-slate-300">{{ $outgoingLetter->perihal }}</dd>
                    </div>
                </div>
            </dl>
            ...
            <div class="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm dark:border-slate-800 dark:bg-slate-900">
                ...
                @if($outgoingLetter->file_path)
                    <x-pdf-document-preview :path="$outgoingLetter->file_path" title="PDF Surat Keluar" />
                @else
        ...
        <div class="flex justify-end border-t border-slate-200 pt-6 dark:border-slate-800">
            <form action="{{ route('outgoing-letters.destroy', $outgoingLetter) }}" method="POST" onsubmit="return confirm('Yakin ingin menghapus surat ini?');">
                @csrf
                @method('DELETE')
